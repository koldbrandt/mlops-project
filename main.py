...
class TrainOREvaluate(object):
    ...
    def train(self):
        ...
        created_model = MyAwesomeModel(784, 10, [512, 256, 128])
        train_set, test_set = data.mnist()
        criterion = nn.NLLLoss()
        optimizer = optim.Adam(created_model.parameters(), lr=0.001)
        model.train(created_model, train_set, test_set, criterion, optimizer, epochs=2)
        ...
        torch.save(checkpoint, str(cwd) + '\\checkpoint.pth')
    ...
    def evaluate(self):
        ...
        criterion = nn.NLLLoss()
        test_loss, accuracy = model.validation(saved_model,test_set,criterion)
    ...
    def load_checkpoint(self,filepath):
        cwd = pathlib.Path(__file__).parent.resolve()
        checkpoint = torch.load(str(cwd) + "\\" + filepath)
        saved_model = MyAwesomeModel(checkpoint['input_size'],
